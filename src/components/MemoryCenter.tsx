...
export const MemoryCenter: React.FC<MemoryCenterProps> = ({ 
  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-fade-in relative">
      <div 
        className={`sticky z-30 -mx-4 md:-mx-8 px-4 md:px-8 pt-4 md:pt-6 pb-4 md:pb-6 transition-all duration-300 bg-[var(--bg-primary)]/95 backdrop-blur-md border-b border-[var(--border-main)] rounded-t-2xl md:rounded-t-3xl`}
      ...
      >
        <div className="flex flex-row items-center justify-between gap-4 mb-4">
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 px-4 md:px-6 py-2 md:py-2.5 bg-emerald-500 text-white hover:bg-emerald-600 rounded-xl transition-all duration-300 font-bold text-xs md:text-sm shadow-xl shadow-emerald-500/20 group w-full sm:w-auto ml-auto"
          >
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar -mx-2 px-2 sm:mx-0 sm:px-0">
          {categories.map(cat => (
            <button
              ...
              className={`px-4 md:px-6 py-2 md:py-2.5 rounded-xl md:rounded-2xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all duration-500 border ${
        ...
        <div className={`p-4 md:p-6 rounded-2xl md:rounded-3xl border transition-all duration-300 ${
          isLimitReached 
          ...
          <div className="flex items-center justify-between mb-3 md:mb-4">
            ...
            {isLimitReached && (
              <button 
                onClick={handlePrune}
                disabled={isPruning}
                className="flex items-center gap-1.5 px-3 md:px-4 py-1.5 md:py-2 bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 rounded-lg md:rounded-xl transition-all text-[10px] md:text-xs font-bold border border-amber-500/20"
              >
      ...
      <div className={`p-3 md:p-4 rounded-xl md:rounded-2xl border flex items-start gap-2.5 md:gap-3 bg-emerald-500/5 border-emerald-500/20`}>
      ...
      {isAdding && (
        <div className={`p-6 rounded-2xl border animate-in slide-in-from-top-2 duration-300 bg-[var(--bg-secondary)] border-[var(--border-main)]`}>
          <div className="flex gap-4 mb-4">
            <div className="flex-1">
              ...
              <select
                ...
                className={`w-full p-2.5 rounded-xl border focus:outline-none focus:border-emerald-500 text-sm transition-all bg-[var(--bg-primary)] border-[var(--border-main)] text-[var(--text-primary)]`}
              >
          ...
          <textarea
            ...
            className={`w-full p-4 rounded-xl border focus:outline-none focus:border-emerald-500 resize-none h-32 mb-4 transition-all bg-[var(--bg-primary)] border-[var(--border-main)] text-[var(--text-primary)]`}
            autoFocus
          />
          <div className="flex justify-end gap-3">
            <button 
              onClick={() => { setIsAdding(false); setNewValue(''); }}
              className="px-6 py-2 rounded-xl text-sm font-medium text-gray-500 hover:bg-[var(--bg-primary)] transition-colors"
            >
              {t('cancel')}
            </button>
            <button 
              ...
              className="px-6 py-2 rounded-xl text-sm font-medium bg-emerald-500 text-white hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-500/20 disabled:opacity-50"
            >
              {t('save')}
            </button>
          </div>
      ...
      {isLoading ? (
        ...
        <div className={`p-12 rounded-3xl border border-dashed flex flex-col items-center justify-center text-center border-[var(--border-main)] bg-[var(--bg-secondary)]/30`}>
        ...
        <div className="space-y-4">
          {filteredMemories.map((memory) => (
            <div 
              key={memory.id} 
              className={`group p-6 rounded-2xl border transition-all duration-300 bg-[var(--bg-secondary)] border-[var(--border-main)] hover:border-emerald-500/30 hover:bg-[var(--bg-secondary)]/80 hover:shadow-xl hover:shadow-emerald-500/5`}
            >
              {editingId === memory.id ? (
                <div className="space-y-4">
                  ...
                  <textarea
                    ...
                    className={`w-full p-4 rounded-xl border focus:outline-none focus:border-emerald-500 resize-none h-28 transition-all bg-[var(--bg-primary)] border-[var(--border-main)] text-[var(--text-primary)]`}
                    autoFocus
                  />
                  <div className="flex justify-end gap-3 pt-2">
                    <button 
                      onClick={() => setEditingId(null)}
                      className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all duration-300 text-gray-500 hover:bg-[var(--bg-primary)]`}
                    >
                      <X size={14} />
                      {t('cancel')}
                    </button>
                    <button 
                      ...
                      className="flex items-center gap-2 px-5 py-2 bg-emerald-500 text-white hover:bg-emerald-600 rounded-xl transition-all duration-300 text-xs font-bold shadow-lg shadow-emerald-500/20 disabled:opacity-50"
                    >
                      <Save size={14} />
                      {t('save')}
                    </button>
                ...
                <div className="flex items-start justify-between gap-6">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0">
                    <button 
                      ...
                      className="p-2.5 rounded-xl text-gray-400 hover:text-emerald-500 hover:bg-emerald-500/10 transition-all"
                    ...
                    <button 
                      ...
                      className="p-2.5 rounded-xl text-gray-400 hover:text-red-500 hover:bg-red-500/10 transition-all"
