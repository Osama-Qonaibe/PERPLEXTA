      ...
      <div className="flex flex-col gap-4 w-full my-4 items-start">
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-30 pointer-events-none overflow-hidden select-none">
            <svg className={`w-full h-full max-w-sm max-h-xs ${isFailed ? 'text-rose-500/10' : 'text-accent/20'}`} viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
              <motion.circle cx="100" cy="100" r="4" className={isFailed ? 'fill-rose-400/40' : 'fill-accent-400 '} animate={{ scale: [1, 1.3, 1] }} transition={{ repeat: Infinity, duration: 2 }} />
              <motion.circle cx="40" cy="60" r="3" className={isFailed ? 'fill-rose-400/30' : 'fill-accent-400'} animate={{ scale: [0.9, 1.2, 0.9] }} transition={{ repeat: Infinity, duration: 2.5 }} />
              <motion.circle cx="160" cy="60" r="3" className={isFailed ? 'fill-rose-400/30' : 'fill-accent-400'} animate={{ scale: [1.1, 0.8, 1.1] }} transition={{ repeat: Infinity, duration: 1.8 }} />
              <motion.circle cx="70" cy="150" r="3" className={isFailed ? 'fill-rose-400/30' : 'fill-accent-400'} animate={{ scale: [1, 1.3, 1] }} transition={{ repeat: Infinity, duration: 2.2 }} />
              <motion.circle cx="130" cy="150" r="3" className={isFailed ? 'fill-rose-400/30' : 'fill-accent-400'} animate={{ scale: [0.8, 1.1, 0.8] }} transition={{ repeat: Infinity, duration: 2.7 }} />
          ...
          <div className={`p-3 w-full flex items-center justify-between bg-zinc-950/40 border-b ${isFailed ? 'border-rose-500/10' : 'border-accent/10'} backdrop-blur-sm z-10`}>
            <div className="flex items-center gap-1.5">
              <span className="relative flex h-2 w-2">
                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${isFailed ? 'bg-rose-400' : 'bg-accent'} opacity-75`} />
            ...
            <div className="flex flex-col items-center justify-center p-6 gap-3 select-none text-center z-10 flex-1">
              <div className="relative flex items-center justify-center">
                <motion.div 
                  ...
                  className="w-16 h-16 rounded-full border border-dashed border-accent/40 flex items-center justify-center"
                />
                <motion.div 
                  animate={{ rotate: -360 }}
                  transition={{ duration: 1.5, repeat: Infinity, ease: 'linear' }}
                  className="absolute w-12 h-12 rounded-full border border-t-accent-500 border-r-transparent border-b-accent-500/20 border-l-transparent"
                />
                <div className="absolute text-[10px] font-mono font-black text-accent ">
              ...
              <div className="flex flex-col items-center max-w-[85%] mt-1">
                ...
                <span className="text-[8px] font-black text-accent/60 uppercase tracking-widest mt-0.5">
          ...
          <div className={`p-2.5 w-full bg-zinc-950/50 border-t ${isFailed ? 'border-rose-500/5' : 'border-accent/5'} backdrop-blur-sm z-10 flex items-center justify-between text-[8px] font-mono text-gray-500`}>
