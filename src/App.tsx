  ...
  React.lazy(async () => {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        console.warn(
          `[LazyRetry] Attempt ${attempt} failed for ${name || "chunk"}:`,
          err,
        );
        if (attempt < 3) {
    ...
    throw new Error(
      `Failed to load module ${name || "component"}. Please check network connection.`,
    );
  });

const ChatPage = lazyRetry(() => import("./pages/ChatPage"), "ChatPage");
const RewardsPage = lazyRetry(
  () => import("./pages/RewardsPage"),
  "RewardsPage",
);
const SubscriptionPage = lazyRetry(
...
const BulletinBoardPage = lazyRetry(
  () => import("./pages/BulletinBoardPage"),
  "BulletinBoardPage",
);
const BlogPage = lazyRetry(() => import("./pages/BlogPage"), "BlogPage");
const AdminCommunityPage = lazyRetry(
  () => import("./pages/AdminCommunityPage"),
  "AdminCommunityPage",
);
const AdminAgencyPage = lazyRetry(
  () => import("./pages/AdminAgencyPage"),
  "AdminAgencyPage",
);
const AdminSystemPage = lazyRetry(
  () => import("./pages/AdminSystemPage"),
  "AdminSystemPage",
);
...
const SharedSnapshotPage = lazyRetry(
  () => import("./pages/SharedSnapshotPage"),
  "SharedSnapshotPage",
);
const RecommendationsPage = lazyRetry(
...
const PWAWrapper = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const isSensitive = [
      ...
      const cleanPath = sensitivePath.startsWith("/")
        ? sensitivePath
        : "/" + sensitivePath;
      return (
    ...
    const siteName =
      language === "ar"
        ? siteSettings?.siteNameAr || siteSettings?.siteName
        : siteSettings?.siteName;
    const resolvedSiteName =
      siteName || (language === "ar" ? "بيربليكستا" : "Perplexta");
    const resolvedOGImage = resolveImageUrl(
      siteSettings?.seoImageUrl || "/app-assets/og-image.png",
      "general",
    );
    ...
    if (isSensitive) {
      ...
      const sensitiveTitle =
        language === "ar"
          ? "بيربليكستا - مساحة عمل محصنة"
          : "Perplexta - Secure Workspace";
      const sensitiveDesc =
        language === "ar"
          ? "صفحة آمنة ومحمية من قبل الخوارزميات السيادية لمنصة بيربليكستا."
          : "Secure node with zero crawling, protected under deep local sovereign protocols.";
      ...
      const siteDesc =
        language === "ar"
          ? siteSettings?.siteDescriptionAr || siteSettings?.siteDescription
      ...
      const resolvedKeywords =
        (language === "ar"
          ? siteSettings?.keywordsAr
          : siteSettings?.keywordsEn) || "";

      const routeMatch = dbRouteSeo.find(
        (r) => r.route === currentPath && r.is_active !== false,
      );

      const dbTitle = routeMatch
        ? language === "ar"
          ? routeMatch.title_ar || routeMatch.title_en
          : routeMatch.title_en || routeMatch.title_ar
        : "";
      const dbDesc = routeMatch
        ? language === "ar"
          ? routeMatch.description_ar || routeMatch.description_en
          : routeMatch.description_en || routeMatch.description_ar
        : "";
      const dbKeywords = routeMatch
        ? language === "ar"
          ? routeMatch.keywords_ar || routeMatch.keywords_en
          : routeMatch.keywords_en || routeMatch.keywords_ar
        : "";
      ...
      if (currentPath === "/subscription") {
        pageTitlePart =
          language === "ar"
            ? "خطط الاشتراك والترقيات النخبة"
            : "Premium Elite Subscription Plans";
      } else if (currentPath === "/marketplace") {
        pageTitlePart =
          language === "ar"
            ? "متجر الأكواد ونخب مطالبات الذكاء الاصطناعي"
            : "Elite Prompts & Advanced Software Marketplace";
      } else if (currentPath === "/blog") {
        pageTitlePart =
          language === "ar"
            ? "المدونة التقنية والتقارير الاستخباراتية"
            : "Tech Intelligence Blog & Decoded Publications";
      } else if (currentPath === "/about") {
        pageTitlePart =
          language === "ar"
            ? "من نحن ورؤية بيربليكستا السيادية"
            : "About Our Sovereign High-Precision Framework";
      } else if (currentPath === "/terms") {
        pageTitlePart =
          language === "ar"
            ? "شروط الخدمة والاتفاقية الرقمية"
            : "Terms of Service";
      } else if (currentPath === "/privacy") {
        pageTitlePart =
          language === "ar"
            ? "سياسة الخصوصية وحقوق حماية البيانات"
            : "Strict Privacy & Data Security Regulations";
      ...
      const routeOGImage = rawOGImage.startsWith("/")
