...
export const AdminCommunityPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"blog" | "marketplace" | "ads">(
    "blog",
  );
  ...
  const [blogTitleEn, setBlogTitleEn] = useState(
    () => localStorage.getItem("draft_blogTitleEn") || "",
  );
  const [blogTitleAr, setBlogTitleAr] = useState(
    () => localStorage.getItem("draft_blogTitleAr") || "",
  );
  const [blogContentEn, setBlogContentEn] = useState(
    () => localStorage.getItem("draft_blogContentEn") || "",
  );
  const [blogContentAr, setBlogContentAr] = useState(
    () => localStorage.getItem("draft_blogContentAr") || "",
  );
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
              ...
              try {
                ...
                if (res.ok) {
                  ...
                  if (data.success && data.file) {
                    const rawUrl =
                      data.file.file_url || data.fileUrl || data.file.url;
      ...
      img.onerror = () => {
        setUploadError(
          language === "ar" ? "ملف الصورة غير صالح." : "Invalid image file.",
        );
        setUploadingImage(false);
      };
  ...
  const handlePublishArticle = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEditing
        ? `/api/blog/articles/${editingArticleId}`
        : "/api/blog/articles";
      ...
      if (res.ok) {
        toast.success(
          language === "ar"
            ? isEditing
              ? "تم تعديل وحفظ المقال بنجاح!"
              : "تم نشر المقال وإرسال إشعارات جماعية بنجاح!"
            : isEditing
              ? "Article updated and saved successfully!"
              : "Article published and global notifications dispatched successfully!",
        );
        cancelEditArticle();
        refreshData();
      } else {
        ...
        toast.error(
          err.message ||
            err.error ||
            (language === "ar" ? "فشلت العملية" : "Publishing failure"),
        );
      }
    } catch (err) {
        ...
        try {
          ...
          if (res.ok) {
            ...
            toast.error(
              err.message ||
                err.error ||
                (language === "ar" ? "فشل الحذف" : "Failed to delete"),
            );
          }
        } catch (err) {
  ...
  if (user?.role !== "admin") {
    return (
      ...
      >
        ...
        <h2 className="text-lg font-black text-[var(--text-primary)] mb-1">
          {isRtl ? "صلاحيات غير كافية" : "Access Restricted"}
        </h2>
        <p className="text-xs text-gray-400 max-w-sm">
          {isRtl
              ...
              <div className="min-h-full flex flex-col pt-[72px] px-6 md:px-8 pb-12 w-full max-w-[1600px] mx-auto">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 pb-6 border-b border-gray-200/5 dark:border-gray-800/10 select-none">
                  ...
                  <div className="flex items-center gap-2 self-start md:self-auto text-[10px] uppercase font-mono tracking-wider text-accent font-bold select-none bg-accent/10 border border-accent/10 px-3 py-1.5 rounded-[4px]">
                    ...
                    <span>
                      {isRtl ? "مشرف معتمد" : "Staff Moderation Mode"}
                    </span>
                  </div>
                </div>
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="bg-[var(--bg-secondary)] border border-[var(--border-main)] rounded-lg p-6 sm:p-8">
                        <h2 className="text-sm font-black text-[var(--text-primary)] mb-6 flex items-center gap-4 border-b border-gray-100/5 pb-3">
                          <div className="flex items-center gap-2">
                            ...
                            {editingArticleId !== null
                              ? isRtl
                                ? "تعديل التقرير أو المقال الاستخباراتي الحالي"
                                : "Edit Intelligence Report / Editorial Article"
                              : isRtl
                                ? "نشر تقرير استخباراتي أو مقال جديد"
                        ...
                        >
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                              <label className="block text-[10px] font-mono uppercase text-gray-450 mb-2">
                                {isRtl
                                  ? "العنوان بالإنجليزية"
                                  : "Title (English)"}
                              </label>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setBlogCategoryEn(e.target.value)
                                }
                            ...
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setBlogCategoryAr(e.target.value)
                                }
                          ...
                          <div className="bg-[var(--bg-base)] border border-[var(--border-main)] rounded-lg p-4">
                            <div className="flex justify-between items-center mb-3">
                              <label className="block text-[10px] font-mono uppercase text-gray-450 tracking-wider font-bold">
                                {isRtl
                                  ? "صورة غلاف المقال (مربعة 1080x1080)"
                                  : "Article Cover Image (Square 1080x1080)"}
                              </label>
                              {blogImageUrl && (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                                ...
                                <label className="relative flex flex-col items-center justify-center h-28 border border-dashed border-gray-200/10 dark:border-gray-800/20 rounded-[4px] bg-[var(--bg-secondary)] hover:bg-[var(--bg-base)] cursor-pointer hover:border-accent/40 hover: transition-theme group overflow-hidden">
                                  ...
                                  {uploadingImage ? (
                                    ...
                                    <div className="flex flex-col items-center gap-1.5 text-center p-3 select-none">
                                      <Upload
                                        size={20}
                                        className="text-gray-400 group-hover:text-accent group-hover:scale-105 transition-theme"
                                      />
                                      <span className="text-[10px] font-black font-sans text-gray-500 group-hover:text-gray-250 transition-colors">
                                        {isRtl
                                          ? "رفع من الجهاز (كروب مربع)"
                                          : "Upload from device (Auto crop)"}
                                      </span>
                              ...
                              <div className="md:col-span-6 space-y-2">
                                <span className="block text-[9px] font-mono uppercase text-gray-500">
                                  {isRtl
                                    ? "أو أدخل رابطاً مباشراً للصورة:"
                                    : "Or supply raw direct image link:"}
                                </span>
                                <input
                                  type="url"
                                  value={blogImageUrl}
                            ...
                            {uploadError && (
                              <p className="mt-2 text-[10px] font-mono text-rose-500">
                                {uploadError}
                              </p>
                            )}
                                ...
                                <div className="min-w-0 flex-1">
                                  <span className="block text-[9px] font-mono text-accent">
                                    {isRtl
                                      ...
                                      : "Cover fully optimized"}
                                  </span>
                                  <span className="block text-[9px] text-gray-400 truncate max-w-sm">
                                    {blogImageUrl}
                                  </span>
                                </div>
                                <button
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <input
                                ...
                                className="w-full h-10 bg-[var(--bg-base)] border border-[var(--border-main)] text-[var(--text-primary)] rounded-sm px-3 text-xs"
                              />
                              <input
                                  ...
                                  <span>
                                    {editingArticleId !== null
                                      ? isRtl
                                        ...
                                        : "Save & Compile Updates"
                                      : isRtl
                                        ? "نشر المقال وإرسال التنبيهات"
                                        : "Publish Article & Notify Users"}
                                  </span>
                                </>
                              )}
                            </button>
                      ...
                      <div className="bg-[var(--bg-secondary)] border border-[var(--border-main)] rounded-lg p-6 sm:p-8">
                        ...
                        {loading ? (
                          ...
                          <p className="text-xs text-gray-500 text-center select-none py-4 font-sans font-medium">
                            {isRtl
