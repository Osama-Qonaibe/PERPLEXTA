...
export async function runDatabaseMigrations() {
  try {
    ...
    // Ensure plans table exists with original structure
    await pool.query(`
    ...
    `);
    await ensureColumn(pool, 'plans', 'is_active', 'BOOLEAN', 'true');
    await ensureColumn(pool, 'plans', 'monthly_price', 'DECIMAL(10, 2)', '0.00');
    await ensureColumn(pool, 'plans', 'annual_price', 'DECIMAL(10, 2)', '0.00');
    await ensureColumn(pool, 'plans', 'discount', 'INTEGER', '0');
    await ensureColumn(pool, 'plans', 'features', 'JSONB', "'[]'");
    await ensureColumn(pool, 'plans', 'limits', 'JSONB', "'{}'");
...
export async function initDb(mode: 'scratch' | 'additive' = 'additive', customPool?: any, customLedgerPool?: any) {
  ...
  if (adminCheck.rows.length === 0) {
    ...
    const newAdmin = await targetPool.query("INSERT INTO users (email, name, password_hash, role, status) VALUES ($1, 'Master Admin', $2, 'admin', 'active') RETURNING id", [adminEmail, adminHash]);
  ...
  if (parseInt(planCheck.rows[0].count) === 0) {
    ...
    await targetPool.query(`
      ...
      ('Professional', 'المحترف', 29.00, 290.00, 15, '["Advanced Analysis", "Unlimited Chats", "Priority Support"]', '#3b82f6', true),
      ('Enterprise', 'المؤسسات', 99.00, 990.00, 20, '["Full Sovereign Access", "Multi-model Orchestration", "Concierge Support"]', '#8b5cf6', false)
