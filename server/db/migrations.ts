...
export async function initDb(mode: 'scratch' | 'additive' = 'additive', customPool?: QueryClient, customLedgerPool?: QueryClient) {
  ...
  const indexes = [
    ...
    { pool: targetPool, query: `CREATE INDEX IF NOT EXISTS idx_posts_title_fts ON forum_posts USING GIN(to_tsvector('english', title))` },
    { pool: targetPool, query: `CREATE INDEX IF NOT EXISTS idx_posts_content_fts ON forum_posts USING GIN(to_tsvector('english', content))` },
    { pool: targetPool, query: `CREATE INDEX IF NOT EXISTS idx_articles_title_fts ON blog_articles USING GIN(to_tsvector('english', title_en))` },
    { pool: targetPool, query: `CREATE INDEX IF NOT EXISTS idx_articles_content_fts ON blog_articles USING GIN(to_tsvector('english', content_en))` },
    { pool: targetPool, query: `CREATE INDEX IF NOT EXISTS idx_forum_posts_category_id ON forum_posts(category_id)` },
    { pool: targetPool, query: `CREATE INDEX IF NOT EXISTS idx_forum_comments_post_id ON forum_comments(post_id)` },
    { pool: targetPool, query: `CREATE INDEX IF NOT EXISTS idx_blog_comments_article_id ON blog_comments(article_id)` },
    { pool: targetPool, query: `CREATE INDEX IF NOT EXISTS idx_blog_ratings_article_id ON blog_ratings(article_id)` },
