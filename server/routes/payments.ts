        ...
        if (session.metadata?.type === 'deposit') {
          ...
          if (userId && itemId) {
            try {
              ...
              await fulfillMarketplacePurchase(userId, itemId, licenseType || 'standard', referralCode || null, Number(pricePaid || 0));
            } catch (err: any) {
              console.error('[Stripe Webhook] Marketplace fulfillment failed:', err);
            }
          }
